...
public class DictControllerTest {
	...
	public void get() throws Exception {
		MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/dict/3").accept(MediaType.APPLICATION_JSON)).andReturn();
	...
	public void list() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.post("/dict/type/unit").contentType(MediaType.APPLICATION_JSON).content(json)).andReturn();
		int status = mvcResult.getResponse().getStatus();
		String content = mvcResult.getResponse().getContentAsString();
		System.out.println(status + "--" + content);
	}
